...
## RULE 1: CURRENCY FORMAT (PHP PRIMARY)

PHP is always the primary currency. ILS and USD appear below or in parentheses.

**Required format for every price mention:**
```
PHP 32,500,000
(כ-1,366,400 שקל / כ-$465,640, שער יציג 14/04/2026)
```

- PHP amount: always with commas, full number
- ILS/USD: prefix with "כ-" (Hebrew) or "approx." (English)
- Rate date: always shown as DD/MM/YYYY
- Source: /config/fx_today.json (updated daily by fx_rates.py)
- Never hardcode ILS or USD values. Always derive from fx_today.json.

**In HTML artifacts:** read fx_today.json on load. Never embed static ILS/USD numbers.
